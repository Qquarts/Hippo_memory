...
class HHSomaQuick:
    ...
    def step(self, dt, I_ext=0.0, ATP=100.0, **kwargs):
        ...
        return {
            ...
            "J_use": 0.0,  # 간소화 모델이므로 ATP 소비는 0으로 설정
            "INa": 0.0,
            "IK": 0.0,
            "IL": 0.0,
            "I_pump": 0.0
    ...
    def set_I_pump_scale(self, scale: float):
        """ATP에 따른 펌프 효율 조절 (HHSoma 호환성)"""
        # HHSomaQuick은 간소화된 모델이므로 I_pump_scale을 무시하거나 저장만 함
        pass

    def update_reversal_potentials(self, ionflow):
        """IonFlow 기반 역전위 업데이트 (HHSoma 호환성)"""
        # HHSomaQuick은 고정 역전위를 사용하므로 이 메서드는 no-op
        pass
